...
final class ImageDownloadManager {
    ...
    lazy var imageDownloadQueue: OperationQueue = {
        ...
        queue.name = "ankitha.BourneDigital.imageDownloadqueue"
    ...
    private init () {}
    func downloadImage(_ photo: String, indexPath: IndexPath?, handler: @escaping ImageDownloadHandler) {
        self.completionHandler = handler
        guard let url = URL(string: photo) else {
            let error = NSError.init(domain: "", code: 404, userInfo: nil)
            self.completionHandler?(nil, nil, indexPath, error)
            return
        }
        if let cachedImage = imageCache.object(forKey: url.absoluteString as NSString) {
           ...
           self.completionHandler?(cachedImage, url, indexPath, nil)
            ...
            if let operations = (imageDownloadQueue.operations as? [PGOperation])?.filter({
                $0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true
    ...
    // FUNCTION to reduce the priority of the network operation in case the user scrolls and
    //  an image is no longer visible.
    func slowDownImageDownloadTaskfor (_ photo: String) {
        guard let url = URL(string: photo) else {
            return
        }
        if let operations = (imageDownloadQueue.operations as? [PGOperation])?.filter({
